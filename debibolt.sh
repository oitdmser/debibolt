...
# set datadir
bitcoin_dir="/path/to/bitcoin"
lnd_dir="/pat/to/lnd"
rpc_user="USERNAME"
rpc_pw="SuperSecretPWGeneratedByrpauth.py"

#Misc  network name might be etho or enopxx depending on whether you have Debian predictable network naming enabled
network_adapter="" 
...
# get storage OLD SECTION USED ON RASPiBOLT TO CHECK SD CARD
...
/usr/local/bin/lncli --macaroonpath=${lnd_dir}/data/chain/bitcoin/mainnet/readonly.macaroon --tlscertpath=${lnd_dir}/tls.cert getinfo 2>&1 | grep "Please unlock" >/dev/null
wallet_unlocked=$?
if [ "$wallet_unlocked" -eq 0 ] ; then
 ...
 ln_alias="$(/usr/local/bin/lncli --macaroonpath=${lnd_dir}/data/chain/bitcoin/mainnet/readonly.macaroon --tlscertpath=${lnd_dir}/tls.cert --tlscertpath=${lnd_dir}/tls.cert getinfo | jq -r '.alias')" 2>/dev/null
 ln_walletbalance="$(/usr/local/bin/lncli --macaroonpath=${lnd_dir}/data/chain/bitcoin/mainnet/readonly.macaroon --tlscertpath=${lnd_dir}/tls.cert --tlscertpath=${lnd_dir}/tls.cert walletbalance | jq -r '.confirmed_balance')" #2>/dev/null
 ln_channelbalance="$(/usr/local/bin/lncli --macaroonpath=${lnd_dir}/data/chain/bitcoin/mainnet/readonly.macaroon --tlscertpath=${lnd_dir}/tls.cert --tlscertpath=${lnd_dir}/tls.cert channelbalance | jq -r '.balance')" 2>/dev/null
	
fi

ln_channels_online="$(/usr/local/bin/lncli --macaroonpath=${lnd_dir}/data/chain/bitcoin/mainnet/readonly.macaroon --tlscertpath=${lnd_dir}/tls.cert getinfo | jq -r '.num_active_channels')" 2>/dev/null
ln_channels_total="$(/usr/local/bin/lncli --macaroonpath=${lnd_dir}/data/chain/bitcoin/mainnet/readonly.macaroon --tlscertpath=${lnd_dir}/tls.cert listchannels | jq '.[] | length')" 2>/dev/null
ln_external="$(/usr/local/bin/lncli --macaroonpath=${lnd_dir}/data/chain/bitcoin/mainnet/readonly.macaroon --tlscertpath=${lnd_dir}/tls.cert getinfo | jq -r '.uris[0]' | tr "@" " " |  awk '{ print $2 }')" 2>/dev/null
